 ...
 * * [향후 개발 필요 사항 (TODO)]
 * 1. LLM API 연동: fetchSentencesFromLLM 함수 내부 구현 필요
...
// 화면 상태 관리 (0:스플래시, 1~2:질문, 3:로딩, 4:선택, 5:꾸미기, 6:생성, 7:결과)
let currentState = 0; 
...
function setup() {
  ...
  // button _ html
  resetBTN = createButton("🔄 새로고침");
  resetBTN.position(width/2 - 150, 2080);
  resetBTN.size(160, 60);
  ...
  mouseOvercss(resetBTN, 160, 60);

  createBTN = createButton("✅ 이대로 완성");
  createBTN.position(width/2 + 150, 2080);
  ...
  for (let i = 0; i < customMood.length; i++) {
    ...
    moodBTN.position(20 + (i%5)*(totalW/5), 370 + int(i/5)*70);
    ...
    moodBTN.is_pressed = false;
    mouseOvercss(moodBTN, totalW/5 - 15, 55)
    mouseClcikedcss(moodBTN, totalW/5 - 15, 55)
    moodBTN.hide()
  ...
  for (let i = 0; i < customColor.length; i++) {
    ...
    colorBTN.position(20 + (i%5)*(totalW/5), 570 + int(i/5)*70);
    ...
    colorBTN.is_pressed = false;
    mouseOvercss(colorBTN, totalW/5 - 15, 55)
    mouseClcikedcss(colorBTN, totalW/5 - 15, 55)
    colorBTN.hide()
  ...
  for (let i = 0; i < customFont.length; i++) {
    ...
    fontBTN.position(20 + (i%5)*(totalW/5), 800 + int(i/5)*70);
    ...
    fontBTN.is_pressed = false;
    mouseOvercss(fontBTN, totalW/5 - 15, 55)
    mouseClcikedcss(fontBTN, totalW/5 - 15, 55)
    fontBTN.hide()
    customFontbuttons.push(fontBTN); // 배열에 저장
  }

  cameraButton = createButton(`📷 \n 눌러서 사진 촬영`)
  cameraButton.position(20, 1020);
  cameraButton.size(totalW, 150);
  cameraButton.style("font-size", "22px");
  cameraButton.style("border-radius", "15px");
  //cameraButton.mousePressed(handleCameraButton); // 함수 만들면 주석 해제 
  cameraButton.hide()
}
...
function draw() {
  ...
  if ([1, 2, 4, 5].includes(currentState)) {
    drawNavButtons();
  }
...
function mousePressed() {
  ...
  if (currentState === 0) changeState(1);
    ...
    if (currentOptions) {
      ...
      for (let i = 0; i < currentOptions.length; i++) {
        if (isMouseOverButton(width/2, startY + (i * gap), 700, 70)) {
          ...
          // 백엔드 데이터 요청
          isDataLoaded = false; 
          fetchSentencesFromLLM(userSelections); 
    ...
    // 문장 선택
    if (isMouseOverButton(width/2, 250, 700, 80)) { selectedSentence = recommendedSentences[0]; goCustom(); }
    else if (isMouseOverButton(width/2, 350, 700, 80)) { selectedSentence = recommendedSentences[1]; goCustom(); }
    else if (isMouseOverButton(width/2, 450, 700, 80)) { selectedSentence = recommendedSentences[2]; goCustom(); }
  }
...
function goBack() {
  ...
  else if (currentState === 4) changeState(2); 
  else if (currentState === 5) changeState(4);
}
